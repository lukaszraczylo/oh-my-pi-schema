...
coverage 4/5 committed (80%) · state keys [buildGreen, failing] · is_goal false · GREEN
```
...
2. The harness replays from `initialState()`, applying `step(state, action)` per entry. `predict: null` counts as skipped; anything else is compared verbatim against `digest(observation, action)`. Coverage counts only billable transitions: made with a gated, world-changing tool, and from the current session when `schema.coverageScope` is `session`. A `digest` that returns `null` skips the entry.
3. The verdict is cached against the model hash, the timeline length, and the billing key (coverage scope, session id, and gated tools), and the newest ledger revision is stamped with the state keys, the green flag, and the mismatch count.
