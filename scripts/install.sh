...
install_via_bun() {
    ...
    echo ""
    echo "✓ Installed omps via bun"
    echo "Run 'omps' to get started!"
}
...
install_binary() {
    ...
    curl -fsSL --connect-timeout 10 --speed-limit 1024 --speed-time 30 "$BINARY_URL" -o "${INSTALL_DIR}/omps"
    chmod +x "${INSTALL_DIR}/omps"
    ...
    if ! SMOKE_OUTPUT="$("${INSTALL_DIR}/omps" --version 2>&1)"; then
        echo ""
        echo "✗ omps was downloaded to ${INSTALL_DIR}/omps but cannot start:"
        echo "$SMOKE_OUTPUT" | sed 's/^/    /'
        if [ "$PLATFORM" = "linux-musl" ]; then
            echo ""
            echo "The musl build links libstdc++/libgcc dynamically. Install them, then re-run 'omps':"
    ...
    echo ""
    echo "✓ Installed omps to ${INSTALL_DIR}/omps"

    # Check if in PATH
    case ":$PATH:" in
        *":$INSTALL_DIR:"*) echo "Run 'omps' to get started!" ;;
        *) echo "Add ${INSTALL_DIR} to your PATH, then run 'omps'" ;;
    esac
}
