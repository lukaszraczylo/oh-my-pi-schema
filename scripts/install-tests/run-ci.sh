...
mkdir -p "$BINARY_DIR"
cp packages/coding-agent/dist/omps "$BINARY_DIR/omps"
smoke_cli "$BINARY_DIR/omps"
...
(
   ...
   smoke_cli "$BUN_INSTALL/bin/omps"
)
...
#    `bin.omps` from `src/cli.ts` to the prepack bundle `dist/cli.js`. The repo
...
(
   ...
   smoke_cli ./node_modules/.bin/omps
)
