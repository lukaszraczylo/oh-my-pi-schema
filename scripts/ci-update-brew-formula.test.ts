...
const SUMS = {
	"omps-darwin-arm64": "darwin_arm64_sha",
	"omps-darwin-x64": "darwin_x64_sha",
	"omps-linux-arm64": "linux_arm64_sha",
	"omps-linux-x64": "linux_x64_sha",
};
...
describe("renderFormula", () => {
	...
	// outside the staging CWD, `Dir["omps-*"].first` returns `nil`, and
	...
	it("attaches `using: :nounzip` to every per-platform url stanza", () => {
		...
		for (const arch of ["omps-darwin-arm64", "omps-darwin-x64", "omps-linux-arm64", "omps-linux-x64"]) {
	...
	it("wraps `generate_completions_from_executable` with a HOME redirect to buildpath", () => {
		expect(formula).toMatch(
			/with_env\(HOME: buildpath\) do\n\s+generate_completions_from_executable\(bin\/"omps", "completions", shells: \[:bash, :zsh, :fish\]\)\n\s+end/,
		);
